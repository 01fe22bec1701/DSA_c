...
head=NULL;
 ...
 struct player* min_gl(struct player *head);
 struct player* dy_left_det(struct player *head);
...
printf("14--MINIMUM NO OF GOALS\n");
printf("15--DISPLAY THE DETAILS OF THE LEFT NODE\n");
printf("16--EXIT\n");
...
switch(ch)
{
    ...
    case 13: head=de_aft_given_name(head);
             break;
    case 14: head=min_gl(head);
             break;
     case 15: head=dy_left_det(head);
              break;
...
struct player* search_gl(struct player *head)
{
 ...
 struct player* min_gl(struct player *head)
 {
  ...
  printf("player with minimum goal=%d\n",m);
   printf("name=%s\t goal=%d\t height=%f\n",j->name,j->gl,j->ht);
  return(head);
}
 ...
 struct player* dy_left_det(struct player *head)
 {
  ...
  while(start!=NULL)
  {
      if(strcmp(start->name,n)==0)
      {
          if(start==head)
          {
              printf("no player is present before\n");
              break;
          }
          else
          {
